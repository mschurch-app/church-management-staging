...
let page=0,search='',generation=0,busy=false;
const text=(tag,value)=>{const el=document.createElement(tag);el.textContent=value;return el;};
function clear(){generation++;list.replaceChildren();editor.replaceChildren();$('#new').disabled=true;$('#prev').disabled=true;$('#next').disabled=true;}
function edit(row=null){
 editor.replaceChildren();const form=document.createElement('form'),inputs={};
 form.append(text('h2',row?'編輯會員':'新增測試會員'));
 for(const [key,[label,max]] of Object.entries(FIELDS)){
  ...
  input.maxLength=max;input.value=row?.[key]??'';input.required=key==='name';
...
async function load(){
 ...
 try{
  const result=await listMembers(db,church,search,page);if(ticket!==generation)return;
  $('#new').disabled=false;$('#prev').disabled=page===0;$('#next').disabled=!result.hasNext;
...
$('#search-form').onsubmit=event=>{event.preventDefault();if(busy)return;search=$('#search').value;page=0;load();};
