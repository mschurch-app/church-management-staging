...
成功登記只會把驗證過的 LINE subject 和邀請目標名稱寫入 `pastoral_identity_enrollments`。它不會建立 `pastoral_staff` 或 `pastoral_staff_access`，因此不會開放講章、會友或探訪資料。管理者確認身分和角色後，才可另行加入幕僚授權；完成收件後應撤銷未使用代碼。
