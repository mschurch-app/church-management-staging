...
test('rest days are per staff, with no default rest day', () => {
  const start = taipei('2026-09-28T10:00:00+08:00'); // Monday
  const end = start + 60 * 60 * 1000;
  assert.equal(scheduleError(start, end, false, {}, 0), null);
  assert.equal(scheduleError(start, end, false, {restDays:[1]}, 0), 'rest_day');
...
test('emergency bypass obeys each staff preference and still validates time', () => {
  const start = taipei('2026-09-28T10:00:00+08:00');
  const end = start + 60 * 60 * 1000;
  assert.equal(scheduleError(start, end, true, {restDays:[1]}, 0), null);
  assert.equal(scheduleError(start, end, true, {restDays:[1],allowEmergencyOverride:false}, 0), 'emergency_not_allowed');
  assert.equal(scheduleError(start, end, true, {restDays:[1]}, end + 1), 'meeting_in_past');
});
