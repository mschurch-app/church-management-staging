...
const modules=[
  {file:'admin-dashboard.html',label:'管理首頁',icon:'🏠'},
  {file:'members.html',label:'會友名冊',icon:'👥',permission:'members',also:['member-audit.html','binding-review.html']},
...
function mountModuleBar(access,selected){
  ...
  for(const module of modules.filter(item=>allowed(access,selected,item))){const link=document.createElement('a'),icon=document.createElement('span'),label=document.createElement('span');link.href=moduleUrl(module.file,selected);link.className=moduleIsCurrent(module)?'current':'';icon.className='module-bar-icon';icon.textContent=module.icon;icon.setAttribute('aria-hidden','true');label.textContent=module.label;link.append(icon,label);nav.append(link);}
  anchor.after(nav);
}
...
async function mountManagement(){
  ...
  try{
    ...
    const [{db},{readAccess,chooseChurch}]=await Promise.all([import('./admin-db.mjs'),import('./admin-access.mjs?v=20260923-profile2')]);
    ...
    document.documentElement.dataset.church=selected;
    cleanLegacyNavigation();mountModuleBar(access,selected);mountHeaderTools(access,selected);
