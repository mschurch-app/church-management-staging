...
const PAGE_SIZE=24;
let rows=[],scenarios=[],page=1,busy=false;
...
function scenarioFor(name){return scenarios.find(item=>item.name===name);}
function activeScenarios(){return scenarios.filter(item=>item.is_active);}
...
function openForm(row=null){
  ...
  if(mode==='today'){
    ...
    const category=document.createElement('select'),title=control('text',row?.title||''),caption=control('textarea',row?.share_caption||''),file=control('file'),available=activeScenarios();
    for(const item of available)category.append(new Option(`${item.icon} ${item.name}`,item.name));if(row?.category&&!available.some(item=>item.name===row.category))category.append(new Option(`${scenarioFor(row.category)?.icon||'💛'} ${row.category}（目前關閉）`,row.category));category.value=row?.category||available[0]?.name||'';
    ...
    title.placeholder='圖卡顯示標題';caption.placeholder='輸入經文、出處與祝福文字';
...
function renderScenarios(){if(mode!=='love')return;const area=$('#scenario-list');area.replaceChildren();for(const row of scenarios){const card=element('article','',`scenario-card${row.is_active?'':' is-inactive'}`),identity=element('div','','scenario-identity'),icon=element('span',row.icon,'scenario-icon'),copy=element('div');copy.append(element('strong',row.name),element('small',row.is_active?'LINE 會友目前看得到此情境':'已關閉，相關圖卡仍完整保留'));identity.append(icon,copy);const toggle=element('button',row.is_active?'關閉情境':'重新開放',row.is_active?'ghost-button':'secondary');toggle.type='button';toggle.onclick=()=>changeScenarioState(row,!row.is_active);card.append(identity,toggle);area.append(card);}if(!scenarios.length)area.append(element('div','尚未建立祝福情境。','empty-state'));}
async function addScenario(event){event.preventDefault();if(busy)return;const form=event.currentTarget;setBusy(form,true);try{await saveLoveShareScenario(db,church,{name:$('#scenario-name').value,icon:$('#scenario-icon').value});form.reset();$('#scenario-icon').value='💛';await load({keepScenarioPanel:true});$('#status').textContent='新情境已新增並立即開放。';}catch(error){$('#status').textContent=error.message;setBusy(form,false);}}
async function changeScenarioState(row,active){if(busy)return;const message=active?`重新開放「${row.name}」？`:`關閉「${row.name}」？相關圖卡會暫時從 LINE 隱藏，資料仍會保留。`;if(!confirm(message))return;busy=true;try{await setLoveShareScenarioActive(db,church,row,active);await load({keepScenarioPanel:true});$('#status').textContent=active?'情境已重新開放。':'情境已關閉，相關圖卡已從 LINE 隱藏。';}catch(error){$('#status').textContent=error.message;}finally{busy=false;}}
...
function renderCategories(){const selected=$('#category').value,categories=mode==='love'?[...new Set([...scenarios.map(row=>row.name),...rows.map(row=>row.category)].filter(Boolean))]:[...new Set(rows.map(row=>row.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-Hant'));$('#category').replaceChildren(new Option(mode==='today'?'全部分類':'全部情境',''),...categories.map(value=>new Option(value,value)));if(categories.includes(selected))$('#category').value=selected;}
function render(){const area=$('#items'),matches=filteredRows(),pages=Math.max(1,Math.ceil(matches.length/PAGE_SIZE));if(page>pages)page=pages;const visible=matches.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);area.replaceChildren();
  for(const row of visible){const scenario=scenarioFor(row.category),effective=row.is_active&&(mode!=='love'||scenario?.is_active!==false),card=element('article','','library-card'+(effective?'':' is-inactive'));card.append(element('span',(scenario?.icon?scenario.icon+' ':'')+(row.category||'未分類')+(scenario?.is_active===false?' · 情境已關閉':''),'status-badge'));
  ...
  $('#active-count').textContent=rows.filter(row=>row.is_active&&(mode!=='love'||scenarioFor(row.category)?.is_active!==false)).length;$('#status').textContent=`共 ${matches.length} 筆符合條件，第 ${page} / ${pages} 頁。`;
...
async function load({keepScenarioPanel=false}={}){closeEditor();$('#status').textContent='正在載入內容庫…';try{if(mode==='today'){rows=await listTodaysMessages(db,church);}else{[rows,scenarios]=await Promise.all([listLoveShareCards(db,church),listLoveShareScenarios(db,church)]);}renderCategories();renderScenarios();render();if(keepScenarioPanel)$('#scenario-panel').hidden=false;}catch(error){rows=[];scenarios=[];$('#status').textContent=error.message;}}

$('#title').textContent=churchName()+' · '+(mode==='today'?'給今天的你':'把愛傳出去');$('#settings').href='church-settings.html?church='+encodeURIComponent(church||'');$('#new').onclick=()=>{if(mode==='love'&&!activeScenarios().length){$('#scenario-panel').hidden=false;$('#status').textContent='請先新增或開放至少一個祝福情境。';return;}openForm();};if(mode==='love'){$('#scenarios').onclick=()=>{$('#scenario-panel').hidden=false;$('#scenario-panel').scrollIntoView({behavior:'smooth',block:'start'});};$('#scenario-close').onclick=()=>{$('#scenario-panel').hidden=true;};$('#scenario-form').onsubmit=addScenario;}for(const id of ['#search','#category','#state'])$(id).addEventListener(id==='#search'?'input':'change',()=>{page=1;render();});load();
